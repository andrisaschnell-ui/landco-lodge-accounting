-- =========================================================================
-- Release 1 — Activate the double-entry engine (schema)
-- =========================================================================

-- 1) Source traceability columns on journal_entries
...
-- 2) Balance-check trigger on journal_lines
--    Ensures sum(debit) = sum(credit) per journal_entry (within 0.01 tolerance).
...
BEGIN
  ...
  -- Allow transient state with zero lines (e.g. mid-delete) and skip during
  -- multi-row inserts where the next row will balance things out.
  IF v_count = 0 THEN
  ...
  IF ROUND(v_debit, 2) <> ROUND(v_credit, 2) THEN
    RAISE EXCEPTION
      'Journal entry % is unbalanced: debit=% credit=% (difference=%)',
...
-- 3) Period-lock trigger on journal_entries
--    Hard block: if the period (year, month) of entry_date is closed,
--    no insert/update/delete is permitted, regardless of role.
...
BEGIN
  ...
  IF v_closed IS TRUE THEN
    RAISE EXCEPTION
      'Accounting period %-% is closed. Reopen it in Settings before posting.',
