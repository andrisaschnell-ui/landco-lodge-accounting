...
function parseSheetTransactions(
  ...
  const rowsDates: unknown[][] = XLSX.utils.sheet_to_json(ws, {
    header: 1, defval: null,
  });

  const transactions: ParsedBankTransaction[] = [];

  // Layout: Col 0: Nº Doc, Col 1: Data, Col 2: Nº Cheque/Documento
  ...
  // Find header row containing "Nº" in col 0 and "Data" in col 1
  let dataStart = -1;
  for (let i = 0; i < Math.min(rows.length, 15); i++) {
    ...
    if (c0.includes('N') && (str(r[1]).toUpperCase().includes('DATA') || str(r[1]).toUpperCase().includes('DATE'))) {
      dataStart = i + 2; // Skip sub-header "Doc" row
      break;
    }
  }
...
export function parseBdoBank(file: ArrayBuffer, month: number, year: number): ParsedBdoResult {
  ...
  for (const name of wb.SheetNames) {
    ...
    if (lower.includes('bank control') && lower.includes('mtn')) {
      transactions.push(...parseSheetTransactions(wb, name, 'MZN', month, year));
    } else if (lower.includes('bank control') && lower.includes('usd')) {
